...
def _supports_claim(record: EvidenceRecord, rule: Mapping[str, Any]) -> bool:
    ...
    if rule.get("claim_id") == "UNSOURCED_RANK":
        return record.evidence_type == "RANKING_SOURCE" and bool(re.search(r"No\\.1|日本一", claim, flags=re.IGNORECASE))
    if rule.get("claim_id") == "UNSOURCED_SATISFACTION":
        return record.evidence_type in {"VERIFIED_METRIC", "TESTIMONIAL"} and "満足度" in claim
    return True
