...
def render_authoritative_html(plan: Mapping[str, Any], directives: Mapping[str, Any]) -> str:
    ...
    scene_markup = "".join(
        f'<section class="scene" data-scene="{_esc(scene["id"])}" data-intent="{_esc(scene["intent"])}"><span>{index:02d}</span><h2>{_esc(scene["intent"])}</h2><p>決めるための情報を整理します。</p></section>'
...
<div><small>authored decision</small><h1>{_esc(decision["primary_job"])}のための入口</h1><p class="lead">CompositionPlanの意図から、確認すべき情報を順に案内します。</p></div>
