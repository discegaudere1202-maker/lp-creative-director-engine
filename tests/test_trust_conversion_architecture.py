...
class TrustConversionArchitectureTest(unittest.TestCase):
    def test_rule_file_is_reusable_and_evidence_safe(self):
        ...
        self.assertIn("SERVICE_PROCESS", payload["evidence_slots"])
        self.assertEqual(payload["evidence_selection_logic"], "config/evidence_selection_logic_v1.json")
        self.assertEqual(payload["status"], "REPLICATED_NOT_ENGINE_IMPLEMENTED")
        self.assertEqual(set(payload["rule_status"]), rule_ids)
        self.assertEqual(payload["engine_ready_candidate_rules"], [])
