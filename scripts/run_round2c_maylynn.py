...
def main() -> int:
    ...
    checks = {
        ...
        "captures": captures,
        "html_review_browser_qa": human_review_browser_payload,
        "manual_lp_edit": 0,
        "fake_cta": 0,
    ...
    artifact = {"name": artifact_name, "source_head": source_head, "root": "artifacts/round2c", "includes": ["maylynn_premium_prototype/index.html", "human_review_html/index.html", "human_review_browser_qa/", "company_research_v2.json", "evidence_graph_v2.json", "customer_decision_model_v1.json", "experience_architecture_v2.json", "creative_composition_v2.json", "quality_review_contract_v2.json", "asset_manifest.json", "source_manifest.json", "captures/", "browser_qa.json", "capture_manifest.json", "reports/"], "github_artifact": "NOT_UPLOADED"}
    write(OUT / "artifact_manifest.json", artifact)
    browser_summary = checks["browser_qa"]
    html_review_summary = {"status": human_review_browser_payload["status"], "total": len(human_review_browser_payload["results"]), "pass": sum(item["status"] == "PASS" for item in human_review_browser_payload["results"]), "fail": sum(item["status"] == "FAIL" for item in human_review_browser_payload["results"]), "overflow_max": max((item["horizontal_overflow_px"] for item in human_review_browser_payload["results"]), default=0), "console_errors": sum(len(item["console_errors"]) for item in human_review_browser_payload["results"]), "page_errors": sum(len(item["page_errors"]) for item in human_review_browser_payload["results"]), "request_failures": sum(len(item["request_failures"]) for item in human_review_browser_payload["results"])}
    all_pass = quality["status"] == "PASS" and checks["research"]["status"] == "PASS" and browser_summary["status"] == "PASS" and browser_summary["total"] == 9 and browser_summary["pass"] == 9 and browser_summary["fail"] == 0 and browser_summary["overflow_max"] == 0 and browser_summary["console_errors"] == 0 and browser_summary["page_errors"] == 0 and browser_summary["request_failures"] == 0 and captures["status"] == "PASS" and captures["counts"]["total"] == 18 and html_review_summary["status"] == "PASS" and html_review_summary["pass"] == 2 and html_review_summary["fail"] == 0 and html_review_summary["overflow_max"] == 0 and html_review_summary["console_errors"] == 0 and html_review_summary["page_errors"] == 0 and html_review_summary["request_failures"] == 0
    summary = {"schema_version": "round2c_maylynn_premium_prototype_v1", "status": "PASS" if all_pass else "HOLD", "round": "2C", "source_head": source_head, "company": "maylynn_paint", "quality_architecture_ready": True, "maylynn_implementation_ready": all_pass, "machine_technical_verification": "PASS" if all_pass else "HOLD", "shun_quality_review": "NOT_STARTED", "one_million_yen_pass": "NOT_ASSESSED", "nagi_no_mirai": "NOT_STARTED", "watashi_no_daidokoro": "NOT_STARTED", "browser_qa": browser_summary, "captures": captures["counts"], "artifact": artifact, "html_review": {"path": "human_review_html/index.html", "self_contained": True, "external_asset_dependencies": [], "browser_qa": html_review_summary}, "full_test_attribution": {"path": "reports/full_test_attribution.json", "classification": "PRE_EXISTING", "round2c_regression_count": 0}, "manual_lp_edit": 0, "creative_production_status": "PROTOTYPE_NOT_FINAL", "remaining": ["Shun must review the actual HTML/screenshots/motion before any final quality decision."]}
