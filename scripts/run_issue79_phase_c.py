    ...
    try:
        async with async_playwright() as pw:
            ...
            for width in WIDTHS:
                ...
                metrics=await page.evaluate("""() => {const e=document.querySelector('main > .premium-scene .scene-layered-copy'); const r=e?.getBoundingClientRect(); const t=[...document.querySelectorAll('main > .premium-scene .scene-layered-copy h1,main > .premium-scene .scene-layered-copy h2,main > .premium-scene .scene-layered-copy p')].map(x=>({text:x.innerText,width:x.getBoundingClientRect().width,height:x.getBoundingClientRect().height})); return {scrollWidth:document.documentElement.scrollWidth,clientWidth:document.documentElement.clientWidth,f01:document.body.dataset.issue77F01,copyWidth:r?.width||0,text:t}}""")
