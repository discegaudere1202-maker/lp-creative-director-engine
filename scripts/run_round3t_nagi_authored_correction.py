...
def candidate_html(html: str) -> str:
    ...
    s2_cards = "".join(f'<article class="state-card"><div class="eyebrow">{label}</div><h3><span class="line-chunk">{headline if label != "知る" else "ヒーリングを、"}</span>{"<br class=\"responsive-break\"><span class=\"line-chunk\">まず知りたい。</span>" if label == "知る" else ""}</h3><p>{service}</p></article>' for headline, service, label in [(COPY["s2"]["cards"][0][0], COPY["s2"]["cards"][0][1], "受ける"), (COPY["s2"]["cards"][1][0], COPY["s2"]["cards"][1][1], "学ぶ"), (COPY["s2"]["cards"][2][0], COPY["s2"]["cards"][2][1], "知る")])
