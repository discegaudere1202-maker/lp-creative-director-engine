...
def main():
    ...
    for c,case in CASES.items():
        ...
        contract={"status":"PASS" if scene_count==len(plan["scene_plan"]) and all(x in headings for x in expected_headings) and {x["stage"] for x in ctas}>={"discovery","reassurance","action"} and not leakage and not editorial else "FAIL","scene_count":{"expected":len(plan["scene_plan"]),"rendered":scene_count},"expected_headings":expected_headings,"rendered_headings":headings,"rendered_ctas":ctas,"internal_metadata_visible":False,"internal_leakage":leakage,"editorial_violations":editorial}
