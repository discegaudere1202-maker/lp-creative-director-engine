...
def hero_visual_relationship(media: dict[str, dict[str, Any]]) -> dict[str, Any]:
    """A03 is distinct from A01, so it is editorial context rather than fake zoom."""
    from PIL import Image
    dimensions = {}
    for asset in ("A01", "A03"):
        path = ROOT / media[asset]["local_asset_path"]
        with Image.open(path) as image_file:
            dimensions[asset] = list(image_file.size)
    contract = {
        "schema_version": "visual_relationship_contract_v1",
        "viewport": "V01",
        "visual_relation_type": "SECONDARY_CONTEXT",
        "relation_type": "SECONDARY_CONTEXT",
        "main_asset_id": "A01",
        "detail_asset_id": "A03",
        "master_asset_id": None,
        "source_crop_rect": None,
        "detail_crop_rect": None,
        ...
        "anti_decoration_rule": "secondary photography requires a narrative function",
    }
    return validate_visual_relationship(contract)
...
.hero-secondary-context{position:absolute;z-index:2;right:7%;bottom:11%;width:min(260px,22vw);color:var(--white)}
.hero-secondary-photo{height:176px;overflow:hidden;border-left:2px solid var(--signal)}
.hero-secondary-photo img{transform:none!important;transition:none!important}
.hero-secondary-label{margin:10px 0 0;font-family:var(--latin);font-size:.7rem;letter-spacing:.08em;color:#f0d4a5}
.hero-secondary-role{margin:2px 0 0;font-size:.78rem;line-height:1.55;color:#e4ecec}
...
@media(max-width:760px){.hero-secondary-context{right:18px;bottom:31%;width:42vw}.hero-secondary-photo{height:19svh;min-height:128px}.hero-secondary-label{font-size:.62rem}.hero-secondary-role{font-size:.7rem}}
...
def render_html(creative: dict[str, Any], media: dict[str, dict[str, Any]]) -> str:
    ...
    relation = hero_visual_relationship(media)
    return f'''<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex"><meta name="description" content="小山市｜外壁・屋根｜修繕・塗装。メイリン塗装工務店の現地相談。"><title>メイリン塗装工務店｜いつもの家に、気になるところがある。</title><link rel="preload" as="image" href="{previous.esc(previous.img_url(media["A01"]))}"><style>{B2_STYLE}</style></head><body data-round="2G-B4" data-company="maylynn_paint" data-hero-visual-relation="SECONDARY_CONTEXT"><header class="site-header"><a class="brand" href="#V01">メイリン塗装工務店</a><nav class="header-right"><span>小山市｜外壁・屋根</span><a class="header-cta" href="#V09">相談する →</a></nav></header><main>
<section id="V01" data-viewport-id="V01" class="scene hero" data-visual-relation="SECONDARY_CONTEXT"><div class="hero-media">{img(media, "A01", "日本の戸建て外壁を確認する作業者", "eager")}<figure class="hero-secondary-context" data-main-asset="A01" data-secondary-asset="A03" data-semantic-relation="WHOLE_HOUSE_OBSERVATION → SURFACE_CONDITION"><div class="hero-secondary-photo">{img(media, "A03", "表面の状態を確認する外壁の近接写真")}</div><figcaption class="hero-secondary-label">表面の状態</figcaption><p class="hero-secondary-role">外壁のひびや剥がれを確認</p></figure></div><div class="hero-content">{heading(c["V01"]["headline"], "h1")}<p class="hero-commercial">{previous.esc(c["V01"]["commercial"])}</p><p class="hero-lead">{previous.esc(c["V01"]["lead"])}</p><a class="hero-action" href="#V09">{previous.esc(c["V01"]["primary_action"])}</a></div></section>
