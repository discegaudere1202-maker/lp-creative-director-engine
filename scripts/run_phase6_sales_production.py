...
def stage_report(name, candidates, manifest, registry, rows):
    statuses={s:sum(x.get("sales_status")==s for x in rows) for s in ("SALES_READY","SALES_HOLD","SALES_BLOCKED","SALES_EXCLUDED")}
    generated=sum(bool(x.get("generation_id")) for x in rows)
...
def main():
    ...
    root=Path(args.out); root.mkdir(parents=True,exist_ok=True); allc=load_candidates(args.master); selected=allc[:100]; stages=[]
